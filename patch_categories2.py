...
# ── Fix API call names (simple string replace on whole content) ────────────────
content = ''.join(lines)
content = content.replace('taskCategoryManager.deleteCategory($0)', 'taskCategoryManager.delete($0)')
content = content.replace('taskCategoryManager.updateCategory(category)', 'taskCategoryManager.update(category)')
# Fix the sheet call that wrongly uses NoteFolderManager
...
# ── Replace AddFolderSheet (lines 645-789, 0-indexed 644-788) with iOSAddCategorySheet ──
...
# AddFolderSheet: lines 645-789 (1-indexed) => indices 644-788
# iOSManageCategories comment starts at line 790 => index 789
add_start = 644   # 0-indexed, inclusive
add_end   = 789   # 0-indexed, exclusive (line 790 is the MARK comment for Manage)
...
# iOSManageCategories: lines 790-903 (1-indexed) => indices 789-902; line 904 is #endif
manage_start = 789   # 0-indexed, inclusive
manage_end   = 903   # 0-indexed, exclusive (line 904 = #endif)
