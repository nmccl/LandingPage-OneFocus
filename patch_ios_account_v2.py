#!/usr/bin/env python3
"""
Patch iOSAccountSettingsView to add:
- Email Updates and Analytics toggles in preferencesCard
- Require Sign-In and Biometric Unlock toggles in securityCard
"""
...
def patch(path, find, replace):
    with open(path) as f:
        src = f.read()
    if find not in src:
        print(f"⚠️  Pattern not found: {find[:80]!r}")
        return False
...
# ─── 1. Add email updates + analytics to preferencesCard ─────────────────────
patch(ios_acct,
...
# ─── 2. Add require auth + biometric toggles to securityCard ─────────────────
# Read the current securityCard section
with open(ios_acct) as f:
    src = f.read()

# Find the securityCard and add the toggles before the Change Password button
patch(ios_acct,
...
print("\n✅ iOSAccountSettingsView patched with email updates, analytics, and biometric toggles.")
